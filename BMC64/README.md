...
Connect RST to GND couple of times to get Arduino Pro Micro to programming mode. Notice that COM-port is different in programming mode in Windows. Also notice that programming mode is available only couple of seconds from reset, so you have to time it right.
